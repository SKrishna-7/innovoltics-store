...
//     }
//   };
//   const clearCart = () => setOrders([]);
  
//   return (
//     <OrderContext.Provider
//       value={{ createOrder, getOrders, getOrderById, deleteOrderById, getOrderByOrderId, OrderUpdateStatus, orders, loading, error,clearCart }}
//     >
//       {children}
//     </OrderContext.Provider>
//   );
...
export const OrderProvider = ({ children }) => {
  ...
  const getOrderById = async (userIdParam, guestIdParam, token) => {
    const uid = userIdParam || localStorage.getItem("user_id");
    const gid = guestIdParam || localStorage.getItem("guest_id");
    if (!uid && !gid) {
      setOrders([]);
      setError("No user or guest ID provided.");
      return [];
    ...
    setError(null);
    try {
      const response = await axios.get(`${BASE_URL}/orders/history/get-by-user`, {
        params: { user_id: uid, guest_id: gid },
        headers: { Authorization: `Bearer ${token}` },
      });
      setOrders(response.data || []);
  ...
  const createOrder = async (orderData) => {
    ...
    try {
      ...
      setOrders((prev) => [...prev, { ...orderData, ...response.data }]);
      console.log("Order created:", response.data);
      return response.data; // { message, order_id, status }
    } catch (error) {
      const errMsg = error.response?.data?.detail || "Error creating order";
      setError(errMsg);
      console.error("Create order error:", error);
  ...
  return (
    <OrderContext.Provider value={{ orders, loading, getOrders, getOrderById, createOrder, getOrderByOrderId, deleteOrderById, OrderUpdateStatus, error }}>
      {children}
    </OrderContext.Provider>
  );
};